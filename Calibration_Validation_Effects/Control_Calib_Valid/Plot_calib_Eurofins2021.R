## SolBeePop_ecotox
## Simulation of Eurofins Osmia semi-field study 2 (2021)
## Author: Amelie Schmolke (changed from script by Brenna Kent)
...
run_num <- 951 # run number, simulation number from LHC; 951: best fit to 2021, 1310: best fit to 2019
study_year <- 2021
...
study_nest1<-ggplot()+
  ...
  xlim(start, end)+
  ylim(0,90)+ # adjust y-axis limits for each sim set and study
  xlab("Date") + ylab("Number of post-emergent females")+
  ...
  labs(tag = "C")
#labs(title = paste("Eurofins",study_year,"data, simulation", run_num))
...
study_brood1<-ggplot()+
  ...
  xlim(start, end)+ ylim(0,900)+ # adjust y-axis limits for each sim set and study
