...
##  Manuscript Fig. 1C: comparing three foraging radii in high detail scenario
...
# plot outputs from the three assumed foraging distances for comparison (note: "dist", sce (c))
# simulation and study name # needs to be set to each scenario / study
run_400m <- "Tue2015_SceC_400m_dist_IT" # run ID, from run*.csv file
run_800m <- "Tue2015_SceC_800m_dist_IT" # run ID, from run*.csv file
run_1200m <- "Tue2015_SceC_1200m_dist_IT" # run ID, from run*.csv file # included in output file name
...
# Scenario (a)
sim_400m<-read_csv(paste0(run_400m,".csv"), skip=6)
...
sim_400m$scenario <- "400m"

# Scenario (b)
sim_800m<-read_csv(paste0(run_800m,".csv"), skip=6)
...
sim_800m$scenario <- "800m"

# Scenario (c)
sim_1200m<-read_csv(paste0(run_1200m,".csv"), skip=6)
...
sim_1200m$scenario <- "1200m"

sim_data <- rbind(sim_400m,sim_800m,sim_1200m)
sim_data$scenario <- factor(sim_data$scenario,levels = c('400m','800m','1200m'))
...
brood<-ggplot()+
  ...
  geom_ribbon(data=subset(sim_data,sim_data$scenario == "400m"), 
              aes(x=Date, ymin=min.sum.cells,ymax=max.sum.cells), fill="dodgerblue2", alpha = 0.1)+
  geom_ribbon(data=subset(sim_data,sim_data$scenario == "800m"), 
              aes(x=Date, ymin=min.sum.cells,ymax=max.sum.cells), fill="orchid3", alpha = 0.1)+
  geom_ribbon(data=subset(sim_data,sim_data$scenario == "1200m"), 
  ...
  ylim(0,1050)+ # adjust y-axis limits: max. 4000 for 2014; max. 1200 for 2015 
  xlab("Date") + ylab("Number of brood cells")+
  scale_shape_manual(values = c(1, 2))+
  scale_color_manual(values = c("dodgerblue2","orchid3","black"))+
...
ggsave(paste0('Fig1C_brood_radius_',run_1200m,'.jpg'), brood, dpi=300, width = 7, height = 5)
