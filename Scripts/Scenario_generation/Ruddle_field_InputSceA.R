...
##  Scenario (a): low detail scenario
...
# Refined scenarios assumes sunshine needed for foraging
...
d.weather2 <- read.csv(file=weather.file2, sep=";", dec=",") # for 2015 files
d.weather2$date <- as.Date(d.weather2$Tag, "%d.%m.%Y") # 
d.weather2$DOY <- seq(from=1, to=length(d.weather2$date), by=1) # only for 2015 files (2014 weather files already include a column with DOY)
#d.weather2 <- read.csv(file=weather.file2, skip = 5) # for 2014 files
...
write.csv(sce.out, file=paste0("../SolBeePop_Input/",sce.file,".csv"), row.names=FALSE)
...
write.csv(meta.out, file=paste0("../SolBeePop_Input/",sce.file,"_meta.csv"), row.names=FALSE, quote=FALSE)
