...
###  Scenario (b): medium detail scenario
...
OSR.file <- "OSR_floral_function/OSR_BBCH_S15_01804_Cel2015.csv"
# first date in OSR flowering file as doy (manual input!)
osr.start.doy <- 119

# weather data; csv file needs to list weather data from a single year 
weather.file1 <- NA # all data in a single weather file
weather.file2 <- "S15-01804_field_Tables/Weather_Ruddle_et_al_study_2015_Cel.csv" # closest weather station
...
# Refined scenarios assume sunshine needed for foraging
...
prop.crop <- 0.73 # proportion foraging on crop: set to generic value for scenario (b)
...
# for 2014 files
#d.weather2 <- read.csv(file=weather.file2, sep=";", dec=",") 
...
# for 2015 files
d.weather2 <- read.csv(file=weather.file2, skip = 5) 
...
# for 2014 files
#names(d.weather)[names(d.weather) == 'MAX_TA200'] <- 'T.max' # from weather station 
...
# for 2015 files
names(d.weather)[names(d.weather) == 'Max..air.temp.'] <- 'T.max' # from weather station 
...
# generic time series in scenario (b), set by prop.crop in scenario definitions
...
for(i in 1:(length(sce.out$Prop_foraging_crop))){
  if(i >= min.doy){
    if(i <= study.end){
      prop_sce.out$Prop_foraging_crop[i] <- prop.crop
      dist_sce.out$Prop_foraging_crop[i] <- prop.crop
    }
  }
}
...
write.csv(prop_sce.out, file=paste0("../SolBeePop_Input/",sce.file,"_prop.csv"), row.names=FALSE)
...
meta.out <- data.frame(c("Script_version","Date_created", "Study info file", "Weather_file1","Weather_file2",
                         ...
                         "osr.start.doy","study.end", "prop.crop", "foraging.radius"), 
                       c("Ruddle_field_InputSceB.R", as.character(Sys.Date()), study.info.file,
                         weather.file1, weather.file2, OSR.file, land_cover_prop.file, Q_factor.file,
                         wind.data, min.temp, max.wind, max.precip, max.humid, use.sunshine, 
                         max.forag.hrs, osr.start.doy, study.end, prop.crop, foraging.radius))
colnames(meta.out) <- c("Parameter", "Value")
write.csv(meta.out, file=paste0("../SolBeePop_Input/",sce.file,"_prop_meta.csv"), row.names=FALSE, quote=FALSE)

# distance to land covers
write.csv(dist_sce.out, file=paste0("../SolBeePop_Input/",sce.file,"_dist.csv"), row.names=FALSE)
## meta file: store characterizations set in this script
#### NOTE: update script name if needed! ####
meta.out <- data.frame(c("Script_version","Date_created", "Study info file", "Weather_file1","Weather_file2",
                         ...
                         "osr.start.doy","study.end", "prop.crop", "foraging.radius"), 
                       c("Ruddle_field_InputSceB.R", as.character(Sys.Date()), study.info.file,
                         weather.file1, weather.file2, OSR.file, land_cover_dist.file, Q_factor.file,
                         wind.data, min.temp, max.wind, max.precip, max.humid, use.sunshine, 
                         max.forag.hrs, osr.start.doy, study.end, prop.crop, foraging.radius))
colnames(meta.out) <- c("Parameter", "Value")
write.csv(meta.out, file=paste0("../SolBeePop_Input/",sce.file,"_dist_meta.csv"), row.names=FALSE, quote=FALSE)
